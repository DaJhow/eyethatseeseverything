...
if($chbe['ban_expire'] <= time()) {
    mysql_query("DELETE * FROM bans WHERE user_id = '". $myrow['id'] ."' OR ip = '". $myrow['ip_last'] ."' OR machine_id = '". $myrow['machine_id'] ."'");
}
...
<?php $newuser = mysql_query("SELECT * FROM users WHERE id = '".$myrow['id']."' AND gender = 'M' ");
while($newuse = mysql_fetch_assoc($newuser)){ ?>
                    <div class="content-box red text-center">
                        <div class="title"> Você está Banido </div>
                        <div class="box-body">
                            <div class="banned-user text-center">
                                <div class="banned-notice">Você infelizmente foi <strong>banido</strong> do <?php echo $Holo['name']; ?> Hotel.</div>
                                <div class="ban-information">Poxa <span class="user-style <?php echo $myrow['user_style']; ?>"><?php echo $myrow['username']; ?></span>, você concordou em seguir nossas regras, mas pelo visto fez coisas totalmente ao contrário.</div>
                                <hr>
                                <div class="ban-information"><b>Motivo:</b> <?php echo $chbe['ban_reason']; ?></div>
                                <div><b>Expira:</b> <?php echo date('d/m/Y', $chbe['ban_expire']); ?> ás <?php echo date('H:i:s', $chbe['ban_expire']); ?>.
                                    <hr>
                                    <img style="margin-top: -20px;" src="<?php echo $Holo['avatar'] . $myrow['look']; ?>&amp;head_direction=3&amp;gesture=sad&amp;size=l">
                                </div>
                                <div class="ban-information text-center">
                                    Seu banimento foi na categoria: <b><?php echo $chbe['type']; ?></b>, você não está autorizado acessar o <?php echo $Holo['name']; ?> por outras contas, as mesmas ações de banimento serão repetidas.
                                </div>
                            </div>
                        </div>
                    </div>
<?php } ?>
<?php $newuser = mysql_query("SELECT * FROM users WHERE id = '".$myrow['id']."' AND gender = 'F' ");
while($newuse = mysql_fetch_assoc($newuser)){ ?>
                    <div class="content-box red text-center">
                        <div class="title"> Você está Banida </div>
                        <div class="box-body">
                            <div class="banned-user text-center">
                                <div class="banned-notice">Você infelizmente foi <strong>banida</strong> do <?php echo $Holo['name']; ?> Hotel.</div>
                                <div class="ban-information">Poxa <span class="user-style <?php echo $myrow['user_style']; ?>"><?php echo $myrow['username']; ?></span>, você concordou em seguir nossas regras, mas pelo visto fez coisas totalmente ao contrário.</div>
                                <hr>
                                <div class="ban-information"><b>Motivo:</b> <?php echo $chbe['ban_reason']; ?></div>
                                <div><b>Expira:</b> <?php echo date('d/m/Y', $chbe['ban_expire']); ?> ás <?php echo date('H:i:s', $chbe['ban_expire']); ?>.
                                ...
                                <div class="ban-information text-center">
                                    Seu banimento foi na categoria: <b><?php echo $chbe['type']; ?></b>, você não está autorizada acessar o <?php echo $Holo['name']; ?> por outras contas, as mesmas ações de banimento serão repetidas.
                                </div>
                            </div>
                        </div>
                    </div>
<?php } ?>
                ...
                <div class="col-md-4">
                    <div class="content-box text-center">
                        <div class="box-body">
                            Não há <b>recursos contra Banimento</b>!<hr> Não existem maneiras de retroceder banimentos, como explicamos claramente na nossa <a href="/way"><?php echo $Holo['name']; ?> Etiqueta</a>, cujo você Leu e Aceitou antes de jogar.
                        </div>
                    </div>
                </div>
            </div>
